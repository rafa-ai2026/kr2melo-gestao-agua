(() => {
  ...
  function renderDashboard(block) {
    ...
    return `<section class="hero"><div><p class="eyebrow">KR²MELO v5.1.5</p><h2>${esc(block.name)}</h2><p>${esc(block.address || 'Endereço não informado')} · ${monthLabel(block.month)}</p></div><div><button class="secondary" data-go="leituras">Lançar leituras →</button></div></section>
...
Esta ação remove os apartamentos da competência atual. O histórico já fechado não será alterado.`)) return;
  ...
  // ===================== KR²MELO v5.1.5 — Histórico, auditoria e operação =====================
