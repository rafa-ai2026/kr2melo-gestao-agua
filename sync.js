(() => {
  ...
  function syncConflict(message = 'A copia na nuvem mudou desde a ultima sincronizacao deste aparelho.') {
    const error = new Error(message);
    error.code = 'KR2_SYNC_CONFLICT';
    return error;
  }
  async function pushState(payload, options = {}) {
    const config = await refreshSessionIfNeeded();
    const uid = encodeURIComponent(config.user.id);
    const force = options?.force === true;
    const expected = String(config.remoteUpdatedAt || '');
    const remote = await remoteInfo();
    const stamp = now();
    let row = null;

    if (remote?.updated_at) {
      if (!force && (!expected || remote.updated_at !== expected)) {
        throw syncConflict('A copia na nuvem foi alterada por outro aparelho. Baixe os dados antes de enviar ou confirme uma substituicao manual.');
      }
      if (force) {
        const data = await request(`/rest/v1/kr2melo_sync_state?user_id=eq.${uid}`, {
          method: 'PATCH',
          headers: { Prefer: 'return=representation' },
          body: JSON.stringify({ payload, updated_at: stamp })
        }, config.accessToken);
        row = Array.isArray(data) ? data[0] : null;
      } else {
        const expectedEncoded = encodeURIComponent(expected);
        const data = await request(`/rest/v1/kr2melo_sync_state?user_id=eq.${uid}&updated_at=eq.${expectedEncoded}`, {
          method: 'PATCH',
          headers: { Prefer: 'return=representation' },
          body: JSON.stringify({ payload, updated_at: stamp })
        }, config.accessToken);
        row = Array.isArray(data) ? data[0] : null;
        if (!row) throw syncConflict();
      }
    } else {
      const body = [{ user_id: config.user.id, payload, updated_at: stamp }];
      const data = await request('/rest/v1/kr2melo_sync_state', {
        method: 'POST',
        headers: { Prefer: 'return=representation' },
    ...
    const updatedAt = row?.updated_at || stamp;
    setConfig({ lastPushAt: now(), remoteUpdatedAt: updatedAt });
    dispatch('push', { updatedAt });
    return row;
  }
